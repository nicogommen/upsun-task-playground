...
- Submitting a prompt creates a run card that progresses through `triggering → running → succeeded` (or `failed`) in real time (≤30 s perceived latency to the first state change after submit). *(Met. The `task_complete` intermediate state was removed with Q-iter2-8.)*
- On `succeeded`, the card shows a clickable PR link. *(Met.)* The preview URL is **deferred** from v1: the admin's env-scoped token cannot read the sibling preview env (Q-iter2-8). It returns once a project-scoped authorization ships (§7.4). The preview env itself is built and serves the change, reachable from the GitHub PR comment.
...
- **Finding (informational).** The new task container is in **private beta** and requires a per-project support ticket to enable. Flagged in the playground README (2026-06-05) so anyone reproducing this setup knows to request enablement first.

---

## 10. Remaining work (unblocked, for next pick-up)
